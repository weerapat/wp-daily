...
class Facebook_Login_Public {
	...
	public function add_button_to_login_form() {
		...
		echo apply_filters('fbl/login_button', '<a href="#" class="css-fbl js-fbl" data-redirect="'.$redirect.'" data-fb_nonce="' . wp_create_nonce( 'facebook-nonce' ).'">'. __('<img class="fb-icon" src="http://rabbitworld.ready.co.th/wp-content/plugins/wp-facebook-login/public/img/fb-icon.png">SIGN IN WITH FACEBOOK', $this->plugin_name) .'<img src="'.site_url('/wp-includes/js/mediaelement/loading.gif').'" alt=""/></a>');
	}
